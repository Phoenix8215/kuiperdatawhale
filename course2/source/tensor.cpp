...
namespace kuiper_infer
{
                              ...
                              float padding_value)
  {
    ...
    int rows = this->rows();
    int cols = this->cols();
    int new_rows = this->rows() + pad_rows1 + pad_rows2;
    int new_cols = this->cols() + pad_cols1 + pad_cols2;
    Tensor temp_tensor(channels, new_rows, new_cols);
    temp_tensor.Fill(padding_value);
    // 根据Tensor有没有channel这个维度进行分类
    if (channels == 1)
    {
      ...
      for (int i = 0; i < rows; i++)
      {
        ...
        {
          temp_tensor.data_(i + pad_rows1, j + pad_cols1, channels) = this->data_(i, j, channels);
        }
      }
    }
    else
    {
      ...
      for (int k = 0; k < channels; k++)
      {
        for (int i = 0; i < rows; i++)
        {
          for (int j = 0; j < cols; j++)
          {
            temp_tensor.data_(i + pad_rows1, j + pad_cols1, k) = this->data_(i, j, k);
          }
        }
      }
    }
    this->data_ = temp_tensor.data_;
  }
